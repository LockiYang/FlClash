...
///
/// 准备工作
/// 1. App内启动时（非qucikstart），会初始化AppController
///   initGeo
///   init(homeDirPath)
///   clashLib?.setState 设置currentProfileName
/// 2. App启动时 ClashLib构造函数会调用 service.init
/// 3. 初始化ServiceEngine，执行_service入口点，建立和ClashCore的通信
///
/// App启动时需要更新profile
/// profile保存地址
/// andoird: /data/user/0/com.follow.clash.debug/files/profiles/potato_profile.yaml
/// macos: ~/Library/Application Support/com.follow.clash/profiles/potato_profile.yaml
/// windows:
///
...
/// getStartForegroundParams：原生获取流量和当前配置文件名作为前台通知参数
/// started：原生层VPNService启动后回调， 启动clashCore.startTun 和 通过ReceivePort监听来自 ClashCore 的消息
/// dnsChanged：
///
/// 调用原生
/// start：启动VPNService，通过started回调获取fd，正常启动不会调用
